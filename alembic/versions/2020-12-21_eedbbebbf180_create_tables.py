...
Revision ID: eedbbebbf180
Revises: 
Create Date: 2020-12-21 19:28:51.400966
...
def upgrade():
    ...
    op.create_index(op.f('ix_horses_country_code'), 'horses', ['country_code'], unique=False)
    op.create_index(op.f('ix_horses_father_id'), 'horses', ['father_id'], unique=False)
    op.create_index(op.f('ix_horses_is_born_male'), 'horses', ['is_born_male'], unique=False)
    ...
    op.create_index('index_track_name_country', 'race_tracks', ['race_track_name', 'country_name'], unique=True)
    op.create_index(op.f('ix_race_tracks_country_name'), 'race_tracks', ['country_name'], unique=False)
    op.create_index(op.f('ix_race_tracks_race_track_name'), 'race_tracks', ['race_track_name'], unique=False)
    ...
    sa.Column('ground', sa.Enum('LIGHT', 'SLOW', 'UNKNOWN', 'HEAVY', 'VERY_LOOSE', 'DRY', 'GOOD', 'LOOSE', 'STICKY', 'FAST', 'VERY_HEAVY', 'GOOD_LOOSE', 'VERY_FAST', 'GOOD_LIGHT', 'STANDART', name='unibethorseshowground'), nullable=True),
    ...
    sa.Column('unibet_n', sa.Integer(), nullable=False),
    sa.Column('type', sa.Enum('HURDLE', 'HARNESS', 'STEEPLE_CHASE', 'CROSS_COUNTRY', 'MOUNTED', 'FLAT', 'UNKNOWN', name='unibetracetype'), nullable=True),
    ...
    sa.Column('blinkers', sa.Enum('UNKNOWN', 'NO_BLINKERS', 'BLINKERS', 'AUSTRALIEN_BLINKERS', 'FIRST_TIME_BLINKERS', 'FIRST_TIME_AUSTRALIAN_BLINKERS', name='unibetblinkers'), nullable=True),
    sa.Column('shoes', sa.Enum('SHOD', 'FIRST_TIME_FULLY_UNSHOD', 'FULLY_UNSHOD', 'FIRST_TIME_BACK_UNSHOD', 'BACK_UNSHOD', 'FIST_TIME_FRONT_UNSHOD', 'FRONT_UNSHOD', name='unibetshoes'), nullable=True),
    sa.Column('silk', sa.String(), nullable=True),
    sa.Column('stakes', sa.Integer(), nullable=True),
    sa.Column('music', sa.String(), nullable=True),
    sa.Column('sex', sa.Enum('MALE', 'FEMALE', 'GELDING', 'UNKNOWN', name='unibethorsesex'), nullable=True),
    sa.Column('age', sa.Integer(), nullable=True),
    sa.Column('team', sa.Integer(), nullable=False),
    sa.Column('coat', sa.Enum('UNKNOWN', 'PALOMINO', 'ROUAN', 'WHITE', 'BLACK', 'GREY', 'CHESTNUT', 'DUN', 'BAY', 'DARK_BAY', 'LIGHT_BAY', 'BROWN', 'STRAWBERRY_ROUAN', 'LIVER_CHESTNUT', 'LIGHT_CHESTNUT', 'DARK_CHESTNUT', 'FLAXEN_CHESTNUT', 'COPPER_CHESTNUT', 'PIEBALD_CHESTNUT', 'RUBICANO_CHESTNUT', 'MEDLEY_CHESTNUT', 'AUBERE', 'CHERRY_BAY', 'RUBICANO_BAY', 'GREY_BAY', 'PIEBALD_BAY', 'BLACK_BROWN', 'GREY_BROWN', 'DARK_BROWN', 'CHOCOLATE', 'LIGHT_GREY', 'DARK_GREY', 'GREY_BLACK', 'GREY_CHESTNUT', 'GREY_ROUAN', 'LOUVET', 'PANGARE_NOIR', 'SABINO', 'MEDLEY_BAY', 'MEDLEY_SABINO', name='unibetcoat'), nullable=True),
    ...
    op.create_index(op.f('ix_runners_final_odds'), 'runners', ['final_odds'], unique=False)
    op.create_index(op.f('ix_runners_horse_id'), 'runners', ['horse_id'], unique=False)
    op.create_index(op.f('ix_runners_jockey_id'), 'runners', ['jockey_id'], unique=False)
    op.create_index(op.f('ix_runners_kilometer_record_sec'), 'runners', ['kilometer_record_sec'], unique=False)
    op.create_index(op.f('ix_runners_length'), 'runners', ['length'], unique=False)
    op.create_index(op.f('ix_runners_morning_odds'), 'runners', ['morning_odds'], unique=False)
...
def downgrade():
    ...
    op.execute("DROP TYPE unibethorseshowground;")
    op.execute("DROP TYPE unibetracetype;")
    op.execute("DROP TYPE unibetshoes;")
    op.execute("DROP TYPE unibetblinkers;")
    op.execute("DROP TYPE unibetcoat")