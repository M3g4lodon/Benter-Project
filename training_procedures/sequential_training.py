...
def train_on_n_horses(
    source: Sources,
    winning_model: AbstractWinningModel,
    start_training_at: dt.datetime,
    n_epochs: int,
    n_horses: int,
    ...
    log_dir = os.path.join(
        LOGS_DIR, f"{start_training_at.isoformat()}/{n_epochs}/{n_horses}"
    )
    ...
    if x.size == 0 and x_val.size == 0:
        ...
        training_history.update({"message": message})
        if verbose:
            print(message)
    ...
    if x.size == 0:
        val_loss = winning_model.evaluate(x=x_val, y=y_val, verbose=int(verbose))[0]
...
def train_on_each_horse_with_epochs(
    ...
    start_training_at = dt.datetime.now()
    ...
    for epoch in range(n_epochs):
        ...
        for n_horses in range(max(2, min_horse), max_horse + 1):
            winning_model, training_history_n_horses = train_on_n_horses(
                source=source,
                winning_model=winning_model,
                n_epochs=epoch,
                n_horses=n_horses,
                start_training_at=start_training_at,
                n_epochs_per_n_horses=n_epochs_per_n_horses,
